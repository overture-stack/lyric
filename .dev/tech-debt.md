...
### No `.nvmrc`, so an out-of-range Node version silently breaks type-checking
fix: add a `.nvmrc` (and/or an `engine-strict` setting, or a CI step that checks `node --version` against `packages/data-provider/package.json`'s `engines.node`) so a shell with the wrong Node version is caught immediately instead of producing confusing, unrelated-looking `tsc` errors.
standalone: yes
context: `packages/data-provider/package.json` declares `"engines": { "node": ">=20.0.0" }`, but nothing in the repo enforces it and no `.nvmrc` exists to auto-select a working version. Confirmed 2026-09-15: running under Node v14.21.2 (an nvm environment's plain default, with several newer versions already installed alongside it) produced roughly 15 `tsc --noEmit` errors scattered across files unrelated to whatever was being worked on (`submissionProcessor.ts`, `fileUtils.ts`, `commitSubmissionWorker.ts`, a test fixture), while `eslint` and `mocha` both ran without complaint. Switching to Node 22 made every one of those errors disappear with no other change. Root cause not fully diagnosed (some dependency's type declarations appear to depend on the Node version active at install time), but the fix doesn't require finding it: pinning an in-range version stops it from recurring.
...
standalone: yes
context: A single file's error report can now be fetched three ways: raw JSON (`GET /submission/:submissionId/errors?fileId=`), a dashboard summary grouped by fieldName+reason (`GET .../errors/summary`), and a CSV/TSV download with `Content-Disposition: attachment` (`GET .../errors/download?fileType=csv|tsv`, `submissionController.ts`/`formatFieldErrorsAsDelimitedText` in `submissionUtils.ts`). What's still open is only "all files in one submission at once": zip the per-file CSV/TSV reports together. `jszip` is already a dependency and already used for exactly this response shape in `dictionaryController.ts:74-87`, so this is mostly wiring, not new pattern work. Note line numbers are computed at parse time (`fileUtils.ts:104`) but aren't persisted into `errors`; worth carrying through if the report should reference original file line numbers.
...
resolved: 2026-09-15, added a dedicated per-file summary endpoint alongside the existing raw-JSON one, plus a CSV/TSV download for the detail view. `GET /submission/:submissionId/errors/summary?fileId=` returns `recordsWithErrors` and `errorsByFieldAndReason` (counts grouped by fieldName+reason, aggregated app-side via the new `groupFieldErrorsByFieldAndReason` in `submissionUtils.ts`, using a new lean `submissionRecordsRepository.getErrorsByFileId` that selects only the `errors` column) so a large-submission dashboard view never has to fetch and reduce the full per-record error list client-side. `GET /submission/:submissionId/errors/download?fileType=csv|tsv` serves the same `mapSubmissionRecordErrorsToFieldErrors` output as an `attachment` via `formatFieldErrorsAsDelimitedText` (`csv-stringify`), replacing the pagination that would otherwise have been needed on the raw detail endpoint. The zip-all-files-in-a-submission variant is still open, see the entry below.

### Agentics `synced` SHA in `AGENTS.md` no longer resolved in the local clone
resolved: 2026-09-10, ran the full reconciliation in `upgrading-adoption.md` § 1 against agentics' current `HEAD` (treated like the "no tag" case per `upstream-check.md`'s troubleshooting note, since the SHA was confirmed unresolvable rather than just stale — clone was current and correctly remoted). Diffed `AGENTS.md`/`CLAUDE.md` directly against the canonical template: added 3 new Interaction-parameters bullets, fixed stale "user"→"developer" and "window"→"workspace" wording, added dispatch-table entries for the new `conventions/git.md` and `conventions/initialization.md` (split out of `session-discipline.md`/`AGENTS.md` upstream, content unchanged), removed the now-duplicated inline Initialization section in favor of the dispatch pointer, and fixed one cosmetic wording diff in `CLAUDE.md`. No conflicting/customized content found — everything was a non-conflicting stale-copy fix, applied as one batch. Also surfaced and answered an unanswered initialization question (agent-index registration), recorded in global context rather than here. Tag updated to `0.20.0 | synced: 2d38a6d0bcb99cd84344a616f952a7421eaec119`.
