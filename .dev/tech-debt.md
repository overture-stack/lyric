...
### Error report as a downloadable file (Content-Disposition/attachment), not just JSON
standalone: yes
context: `GET /submission/:submissionId/errors?fileId=` now returns the per-file field-level error report as JSON (`fieldName`, `fieldValue`, `reason`, `message`; normalized from `SubmissionRecordError[]` by `mapSubmissionRecordErrorsToFieldErrors`, `submissionUtils.ts`), resolving the report-format question this entry previously tracked. What's still open is only the download framing: a `Content-Disposition: attachment` response, and, for all files in a submission, a zip of per-file reports (`jszip` is already a dependency and already used for exactly this response shape in `dictionaryController.ts:74-87`). Note line numbers are computed at parse time (`fileUtils.ts:104`) but aren't persisted into `errors`; worth carrying through if a downloadable report should reference original file line numbers.
