...
| `KAFKA_BROKERS`             | Comma-separated list of Kafka broker addresses (e.g. `localhost:9092`). When set, Lyric publishes each committed record to a Kafka topic. Omit to run without Kafka publishing.                                                                                                                                                                                                                                                                                               |                                        |
| `KAFKA_CLIENT_ID`           | Kafka client identifier. Use a unique value per environment to distinguish producers in broker logs.                                                                                                                                                                                                                                                                                                                                                                          | `lyric`                                |
| `KAFKA_TOPIC`               | Kafka topic that committed records are published to. The topic is created automatically on startup if it does not exist. Required when `KAFKA_BROKERS` is set.                                                                                                                                                                                                                                                                                                                |                                        |
...
| `VALIDATOR_CONFIG`          | Defines a configurable endpoint for validating the existence of a record with a specific value. The configuration is provided as an array of objects, where each object must include 'categoryId', 'entityName', and 'fieldName'. 'categoryId' accepts either a category's numeric id or its alias. These entries determine which entity-field combinations are enabled for validation. (Example: `[{"categoryId": "1", "entityName": "sample", "fieldName": "sample_id" }]`) |                                        |
...
- **[Developer Documentation](https://docs.overture.bio/build/core-software/Lyric/overview):** Technical resources for developers working with or contributing to the project, located in the `/docs` folder and from our [developer docs website](https://docs.overture.bio/build/core-software/Lyric/overview).
